...
/**
 * Unique numbers and inspiration from
 * https://github.com/albertoruibal/carballo/blob/master/core/src/main/java/com/alonsoruibal/chess/hash/ZobristKey.java
 *
 * The unique number tables are used to upkeep the board situation as a hash
 * number. A XOR -operation between the pieces on the board in their respective
 * squares should give a unique hash for the current board.
 *
 * @author artkoski
 */
public class Zobrist {
    ...
    /**
     * Keys are board HashKeys, whereas Values are the evaluation score.
     */
    static HashMap<Integer, Integer> transpositionTable = new HashMap<>(3_000);

    static int hashMapSize = 3_000;

    /**
     * The 'unique' numbers for pieces in their respective squares, source in
     * class doc.
     */
    ...
    /**
     * Returns a hash for a piece on a specific square.
     *
     * @param idx - index of the square, for example E2 = index 12
     * @param pieceAtLoc - piece in question in string format
     * @return a hash number
     */
    public static long getKeyForSquare(int idx, String pieceAtLoc) {
        switch (pieceAtLoc) {
            case "WHITE_PAWN":
                return pawn[0][idx];
            case "BLACK_PAWN":
    ...
    public static long getKeyForMove(int from_loc, int to_loc, String piece) {
        return getKeyForSquare(from_loc, piece) ^ getKeyForSquare(to_loc, piece);
    }

    public static long getKeyForMove(Move move, Board b) {
        return getKeyForMove(move.getFrom().ordinal(), move.getTo().ordinal(), b.getPiece(move.getFrom()).value());
    }

    /**
     * XOR -operation between all the pieces on their respective squares.
     *
     * @param b - current board
     * @return hash for the current situation
     */
    public static long getKeyForBoard(Board b) {
        long key = 0;
        long ownPieces = b.getBitboard(b.getSideToMove());
        while (ownPieces != 0L) {
            int pieceIndex = BitOperations.bitScanForward(ownPieces);
            Square squareCurrent = Square.squareAt(pieceIndex);

            key ^= getKeyForSquare(pieceIndex, b.getPiece(squareCurrent).value());

            if (b.getSideToMove().equals(Side.WHITE)) {
                key ^= whiteMove;
            }
            ownPieces = BitOperations.removeLSB(ownPieces);
        }
        return key;
    }

    /**
     * Used to update the transpositionTable. Calculates the hashKey using
     * modulo hashMapSize.
     *
     * @param hash - current hash
     * @param score - evaluation for current hash
     */
    public static void updateHash(long hash, int score) {
        int hashKey = (int) (hash % hashMapSize);
        transpositionTable.put(hashKey, score);
    }

    public static Integer getScoreFromHash(long hash) {
        int hashKey = (int) (hash % hashMapSize);
